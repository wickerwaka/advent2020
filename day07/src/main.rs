...
impl AdventParse for BagType {
    fn parse(line: &str) -> Result<Self, Error> {
        ...
        for segment in words.split(|w| *w == "bags" || *w == "bag" || *w == "contains" || *w == "contain" || *w == "" ) {
            if segment.len() == 0 {
                continue;
            }
            if name.is_none() {
                ...
                match segment {
                    &[ "no", "other" ] => {},
                    &[ x, y ] => { bag_groups.push( BagGroup{ color: format!( "{} {}", x, y), count: 1 } ) },
                    &[ s_count, x, y] => { 
                        let count : usize = s_count.parse()?;
                        bag_groups.push( BagGroup{ color: format!( "{} {}", x, y), count } );
                    }
                    _ => {}
                };
            }
        ...
        Ok(BagType{ color: name.unwrap(), groups: bag_groups })
    }
}
...
fn main() -> Result<(), Error> {
    ...
    let count = bag_types.iter().filter(|x| contains("shiny gold", &x.color, &bag_map)).count();
